# SLT 203.4: I Can Build a Transaction That Passes Input Data to a Smart Contract Using a Redeemer

You have already attached a datum to a script output in 203.3. Before you unlock those funds in 203.5 and mint with a Plutus validator in 203.6, you need to understand redeemers, what they are structurally, the three common patterns you will encounter in blueprints, and exactly how to construct `PlutusData` in Apollo.
...
- Completed 203.3 (lock transaction — you created the UTxO you will unlock in 203.5)
...
For **spending** (to be used in 203.5):
```go
...
In 203.5 you will use the spend redeemer to unlock the funds you locked in 203.3. In 203.6 you will use the mint redeemer to mint tokens with that same hello_world Plutus validator.
