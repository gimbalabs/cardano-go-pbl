...
[Dingo](https://github.com/blinklabs-io/dingo) is Blink Labs' Go-native Cardano node. It covers the same role as Dolos — syncing the chain, serving Ouroboros over a socket, exposing query APIs — but is written in Go on top of gOuroboros itself.

| Interface | What it gives you |
|-----------|------------------|
| Ouroboros Unix socket | ChainSync + LocalStateQuery — same as Dolos |
| gRPC (UTxORPC) | UTxO queries with pluggable storage backends (SQLite, Postgres, BadgerDB) |
| Built-in block validation | Byron through Conway eras; Plutus V1/V2 validation still in progress |
