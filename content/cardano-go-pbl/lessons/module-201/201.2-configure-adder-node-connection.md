...
> **Running Dingo instead?** The diagram above is identical — same NtN protocol, same socket role. The only thing that changes is the path to the socket file and which process is the "local node" in the middle box. Everything Adder sees on the wire is the same.
