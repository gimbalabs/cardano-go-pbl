...
export function Card({ children, className = "", noHover = true }: CardProps) {
  ...
  return (
    <div
      className={`rounded-sm border border-course-border bg-course-card ${hoverClass} ${className}`.trim()}
    >
      {children}
    </div>
  );
...
export function CardHeader({ children, className = "" }: CardHeaderProps) {
  return (
    <div
      className={`border-b border-course-border px-4 py-4 sm:px-6 ${className}`.trim()}
    >
      {children}
    </div>
  );
