...
export function EvidenceEditor({
  ...
  if (!editor) {
    return (
      <div className="rounded-sm border border-course-border overflow-hidden">
        <div className="flex items-center gap-0.5 border-b border-course-border bg-course-surface/50 px-2 py-1.5">
          {Array.from({ length: 8 }).map((_, i) => (
            <div key={i} className="h-7 w-7 animate-pulse rounded-sm bg-course-surface" />
          ))}
        </div>
        <div className="bg-course-surface p-5">
          <div className="space-y-3">
            <div className="h-4 w-3/4 animate-pulse rounded bg-mn-text/10" />
            <div className="h-4 w-1/2 animate-pulse rounded bg-mn-text/10" />
            <div className="h-4 w-5/6 animate-pulse rounded bg-mn-text/10" />
          </div>
        </div>
      </div>
    );
...
function Toolbar({ editor }: { editor: ReturnType<typeof useEditor> }) {
  ...
  return (
    <div className="flex items-center gap-px border-b border-course-border bg-course-surface/60 px-1.5 py-1">
      {/* Text formatting */}
    ...
    <button
      ...
      className={`
        ...
        ${active
          ...
          : "text-mn-text-muted hover:text-mn-text hover:bg-course-surface"
        }
...
function Sep() {
  return <div className="mx-1 h-5 w-px bg-course-border/60" />;
}
