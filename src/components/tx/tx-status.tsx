...
export function TxStatus({
  ...
  return (
    <div className="space-y-4 rounded-sm border border-course-border bg-course-card p-3 sm:p-4">
      {/* Step list */}
            ...
            >
              ...
              <div className="flex-shrink-0">
                {isComplete ? (
                  ...
                  <div className="flex h-5 w-5 items-center justify-center">
                    <div className="h-1.5 w-1.5 rounded-full bg-course-border" />
                  </div>
                )}
              </div>
      ...
      {txHash && txState === "success" && (
        <div className="border-t border-course-border pt-3">
...
function WatchStatusBadge({ status }: { status: TxWatchStatus }) {
  const config: Record<TxWatchStatus, { label: string; className: string }> = {
    ...
    expired: {
      ...
      className: "text-mn-text-muted bg-course-surface border-course-border",
    },
    stalled: {
