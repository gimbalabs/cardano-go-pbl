 ...
 *
 * MUST be rendered inside a client:only="react" island — uses
 * useTransaction (which imports useWallet from @meshsdk/react).
 */
...
export function ClaimCredential({
      ...
      <Card className="border-mn-primary/30">
        <CardBody className="py-6 text-center">
          ...
          <p className="text-sm text-mn-text-muted">
            You have earned the course credential for {COURSE.title}.
          </p>
        </CardBody>
      </Card>
    );
        ...
        <CardBody className="space-y-4 py-6 text-center">
          ...
          <div>
            ...
            <p className="mb-4 text-sm text-mn-text-muted">
              You have completed all {COURSE.moduleCount} modules. Claim
  ...
  return (
    <Card noHover>
      <CardBody className="py-6 text-center">
        <div className="mb-3 flex justify-center">
          <div className="flex h-12 w-12 items-center justify-center rounded-sm border border-course-border bg-course-surface">
            <TrophyIcon className="h-6 w-6 text-mn-text-muted/40" />
          </div>
        </div>
        <p className="mb-2 text-sm font-medium text-mn-text">
          Complete all {COURSE.moduleCount} modules to claim your
          credential
        </p>
        <p className="text-xs text-mn-text-muted">
          {acceptedCount} of {COURSE.moduleCount} modules accepted
        </p>

        {/* Progress bar */}
        <div className="mx-auto mt-3 h-1.5 w-full max-w-[12rem] overflow-hidden rounded-full bg-course-surface">
          <div
            className="h-full rounded-full bg-mn-primary transition-all duration-500"
            style={{
              width: `${(acceptedCount / COURSE.moduleCount) * 100}%`,
            }}
          />
        </div>
      </CardBody>
